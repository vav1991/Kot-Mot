 ...
 * @since      1.0.0
 */
class JustVideo_Most_Commented_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-justvideo-most-commented widget-posts-thumbnail',
			'description' => __( 'Display most commented posts with thumbnails.', 'justvideo' )
		);

		// Create the widget.
		parent::__construct(
			'justvideo-most-commented', // $this->id_base
			__( '&raquo; Most Commented Posts', 'justvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'justvideo_most_commented_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'justvideo_most_commented_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'justvideo_most_commented_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Commented Posts', 'justvideo' ),
			'limit'     => 5,
			'show_date' => true
		);
