 ...
 * @since      1.0.0
 */
class justvideo_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-justvideo-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'justvideo' )
		);

		// Create the widget.
		parent::__construct(
			'justvideo-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'justvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'justvideo_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'justvideo_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'justvideo_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'justvideo' ),
			'limit'     => 5,
			'show_date' => true
		);
